...
public class SVNBranchTagWizardCopyPage extends SVNWizardPage {
	...
	public SVNBranchTagWizardCopyPage() {
	...
	public void createControl(Composite parent) {
		resource = ((SVNBranchTagWizard)getWizard()).getResource();
		remoteResource = ((SVNBranchTagWizard)getWizard()).getRemoteResource();
	...
	private void showLog() {
		...
		if (((SVNBranchTagWizard)getWizard()).multipleSelections()) {
	        ...
	        try {
	            remoteResource = repository.getRemoteFile(new SVNUrl(((SVNBranchTagWizard)getWizard()).getCommonRoot()));
	        } catch (Exception e) {
		    ...
		    if (resource == null) remoteResource = this.remoteResource;
		        ...
		        try {
		            remoteResource = SVNWorkspaceRoot.getSVNResourceFor(resource).getRepository().getRemoteFile(((SVNBranchTagWizard)getWizard()).getUrl());
		        } catch (Exception e) {
		            MessageDialog.openError(getShell(), Policy.bind("MergeDialog.showLog"), e.toString()); //$NON-NLS-1$
		            return;
		        }
	        ...
	        if (remoteResource == null) {
	            MessageDialog.openError(getShell(), Policy.bind("MergeDialog.showLog"), Policy.bind("MergeDialog.urlError") + " " + ((SVNBranchTagWizard)getWizard()).getUrlText()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	            return;	            
	        }	
		}
