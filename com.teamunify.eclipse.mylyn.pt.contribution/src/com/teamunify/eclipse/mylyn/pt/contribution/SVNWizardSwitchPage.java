...
public class SVNWizardSwitchPage extends SvnWizardDialogPage {
  ...
  public void createControls(Composite parent) {
    ...
    if (commonRoot != null) {
      ...
      urlCombo.setText(SwitchToBranchAction.svnUrl);
    }
  ...
  protected void showLog() {
    ...
    try {
      remoteResource = SVNWorkspaceRoot.getSVNResourceFor(resources[0]).getRepository()
                                       .getRemoteFile(new SVNUrl(urlCombo.getText()));
    } catch (Exception e) {
    ...
    if (remoteResource == null) {
      MessageDialog.openError(getShell(),
                              Policy.bind("MergeDialog.showLog"), Policy.bind("MergeDialog.urlError") + " " + urlCombo.getText()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
      return;
    }
  ...
  private boolean canFinish() {
    return urlCombo.getText().length() > 0
           && (headButton.getSelection() || (revisionText.getText().trim().length() > 0));
  }
  ...
  @Override
  public void createButtonsForButtonBar(Composite paramComposite, SvnWizardDialog paramSvnWizardDialog) {
